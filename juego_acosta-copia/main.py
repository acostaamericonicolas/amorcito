...
def crear_rival_nivel_2():
    ...
    if len(rivales_group) < MAX_RIVALES:  # Verificar la cantidad actual de rivales
        ...
        if timer >= FRECUENCIA_RIVALES:  # Controlar la frecuencia de aparición de los rivales
            ...
            # Crear un auto rival con una imagen aleatoria
            if random.random() < 0.5:
                rival = Rival_2(image_folder + "autoblanco.png", SIZE_RIVAL, (x, y))
            elif random.random() >= 0.5 and random.random() < 0.8:
                rival = Rival_2(image_folder + "autoverde.png", SIZE_RIVAL, (x, y))
            else:
                rival = Rival_2(image_folder + "autoamarillo.png", SIZE_RIVAL, (x, y))
            rivales_group.add(rival)
...
def crear_rival_nivel_3():
    ...
    if len(rivales_group) < MAX_RIVALES:  # Verificar la cantidad actual de rivales
        ...
        if timer >= FRECUENCIA_RIVALES:  # Controlar la frecuencia de aparición de los rivales
            ...
            # Crear un auto rival con una imagen aleatoria
            if random.random() < 0.5:
                rival = Rival_3(image_folder + "autoblanco.png", SIZE_RIVAL, (x, y))
            elif random.random() >= 0.5 and random.random() < 0.8:
                rival = Rival_3(image_folder + "autoverde.png", SIZE_RIVAL, (x, y))
            else:
                rival = Rival_3(image_folder + "autoamarillo.png", SIZE_RIVAL, (x, y))
            rivales_group.add(rival)
...
def rival_colision():
    ...
    if colisiones:
        ...
        pygame.time.delay(50)  # Pausa el juego durante 1 segundos
...
def start_game():
    global pos_fondo, timer, vidas, score, fondo, fondo_alto, auto
    pos_fondo = 0
    timer = 0
    vidas = 30
    score = 0
    ...
    while True:
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.QUIT:
                ...
                if evento.key == pygame.K_LEFT:
                    ...
                    sound_acelerar.play()
                    if score > 50:
                        auto.image = pygame.image.load(path_auto_negro_mov_izq).convert_alpha()
                    ...
                    sound_acelerar.play()
                    if score > 50:
                        auto.image = pygame.image.load(path_auto_negro_mov_der).convert_alpha()
                        auto.image = pygame.transform.scale(auto.image, (auto.rect.width, auto.rect.height))
                    else:
                    ...
                    sound_acelerar.play()
                    if score > 50:
                        auto.image = pygame.image.load(path_auto_negro_mov_up).convert_alpha()
                        auto.image = pygame.transform.scale(auto.image, (auto.rect.width, auto.rect.height))
                    else:
                    ...
                    sound_acelerar.play()
                    if score > 50:
                        auto.image = pygame.image.load(path_auto_negro_mov_down).convert_alpha()
                        auto.image = pygame.transform.scale(auto.image, (auto.rect.width, auto.rect.height))
                    else:
                        ...
                        for evento_pausa in pygame.event.get():
                            if evento_pausa.type == pygame.KEYDOWN:
                                if evento_pausa.key == pygame.K_SPACE:
                                    flag_pausa = False
                                    sound_inicio2.stop()
                        print("entraste al while")
                        ISTRUCCION_TEXT = get_font(80).render("----PAUSA----" , True,"Black") 
                        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(center=(400, 300))
                        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
                        pygame.display.update()
                        print("se pauso el juego")

                    
            elif evento.type == pygame.KEYUP:
                if score >50:
                    auto.image = pygame.image.load(path_auto_negro).convert_alpha()
                    auto.image = pygame.transform.scale(auto.image, (auto.rect.width, auto.rect.height))    
                else:
        ...
        if score < 30:
            ...
            fondo = pygame.image.load(path_ruta_tarde).convert()
            fondo = pygame.transform.scale(fondo, screen_size)
            fondo_alto = fondo.get_height()
            if flag_nivel_2:
                screen.fill("white")
                NIVEL_TEXT = get_font(100).render("- - - NIVEL 2 - - -" , True,"Black") 
                NIVEL_RECT = NIVEL_TEXT.get_rect(center=(400, 300))
            ...
            fondo = pygame.image.load(path_ruta_noche).convert()
            fondo = pygame.transform.scale(fondo, screen_size)
            fondo_alto = fondo.get_height()
            if flag_nivel_3:
                screen.fill("white")
                NIVEL_TEXT = get_font(100).render("- - - NIVEL 3 - - -" , True,"Black") 
                NIVEL_RECT = NIVEL_TEXT.get_rect(center=(400, 300))
                screen.blit(NIVEL_TEXT, NIVEL_RECT)
                pygame.display.update()
                pygame.time.delay(3000)
        ...
        # Verifico si se necesita reiniciar la posición del fondo
        if pos_fondo >= fondo_alto:
            pos_fondo = 0

        if vidas <= 0:
            #cuando las vidas llegan a cero reiniciar el juego o mostrar "Game Over"
            pos_fondo = 0
            timer = 0
            sound_game_over.play()
            mostrar_game_over()
...
def options():
    while True:
        ...
        ISTRUCCION_TEXT = get_font(20).render("MOVIMIENTO CON LAS FLECHAS DE DIRECCION" , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=80)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
        ISTRUCCION_TEXT = get_font(20).render("A : IZQUIERDA" , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=100)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
        ISTRUCCION_TEXT = get_font(20).render("S : ATRAS." , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=120)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
        ISTRUCCION_TEXT = get_font(20).render("D : DERECHA" , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=140)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
        ISTRUCCION_TEXT = get_font(20).render("W : ADELANTE" , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=160)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)

        ISTRUCCION_TEXT = get_font(20).render("OBJETIVO :  ACUMULAR LA MAOYOR CANTIDAD DE DIAMANTES" , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=200)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
        ISTRUCCION_TEXT = get_font(20).render("                         Y ESQUIVAR LOS AUTOS QUE QUIEREN CHOCARTE" , True,"Black") 
        ISTRUCCION_RECT = ISTRUCCION_TEXT.get_rect(left=50, top=220)
        screen.blit(ISTRUCCION_TEXT, ISTRUCCION_RECT)
